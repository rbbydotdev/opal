...
// DONT FORGET TO SET VERCEL_CLIENT_SECRET in your Cloudflare Workers environment variables!
...
const handleRequest = async (request: Request, env: Env): Promise<Response> => {
  ...
  // Handle Vercel OAuth token exchange - inject client secret
  if (host === "api.vercel.com" && path === "/oauth/access_token" && request.method === "POST") {
    if (!env.VERCEL_CLIENT_SECRET) {
      return new Response("Server configuration error: Vercel client secret not configured", {
        status: 500,
        headers: corsHeaders(origin),
      });
    }
    ...
    params.set("client_secret", env.VERCEL_CLIENT_SECRET);
    ...
    try {
      ...
      return new Response("Vercel OAuth request failed", {
        status: 502,
        headers: corsHeaders(origin),
      });
    }
