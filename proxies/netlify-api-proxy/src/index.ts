...
const ALLOWED_HOSTS = ["api.netlify.com"];
// DONT FORGET TO SET NETLIFY_CLIENT_SECRET in your Cloudflare Workers environment variables!
...
const handleRequest = async (request: Request, env: Env): Promise<Response> => {
  ...
  // Handle Netlify OAuth token exchange - inject client secret
  if (host === "api.netlify.com" && path === "/oauth/token" && request.method === "POST") {
    if (!env.NETLIFY_CLIENT_SECRET) {
      return new Response("Server configuration error: Netlify client secret not configured", {
        status: 500,
        headers: corsHeaders(origin),
      });
    }
    ...
    params.set("client_secret", env.NETLIFY_CLIENT_SECRET);
    ...
    try {
      ...
      return new Response("Netlify OAuth request failed", {
        status: 502,
        headers: corsHeaders(origin),
      });
    }
