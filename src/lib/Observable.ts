 ...
 */
export function emitter<T>(observable: Observable<T>) {
  const subscribe = (observable as any)[__observableEmitter] as (key: keyof T, callback: () => void) => () => void;

  return {
    on: (key: keyof T, callback: () => void) => subscribe(key, callback),
    emit: (key: keyof T) => {
      // Trigger a change by setting the property to itself
...
export function observeMultiple<T extends Record<string, any>>(
  ...
  // Create a simple emitter for subscriptions
  const emitter = new Map<keyof T, Set<() => void>>();

  const subscribe = (key: keyof T, callback: () => void) => {
    if (!emitter.has(key)) {
      emitter.set(key, new Set());
    }
    emitter.get(key)!.add(callback);
    return () => emitter.get(key)?.delete(callback);
  };
  ...
  const obj = new Proxy(target as any, {
    set(target: T, prop: string | symbol, newValue: any): boolean {
      // Handle our special emitter symbol
      if (prop === __observableEmitter) {
        return false; // Don't allow setting the emitter
      }
      ...
      if (oldValue !== newValue) {
        ...
        // Call observer callback if it exists
    ...
    get(target: T, prop: string | symbol): any {
      // Return the subscription function for our special symbol
      if (prop === __observableEmitter) {
        return subscribe;
      }
      return (target as any)[prop];
