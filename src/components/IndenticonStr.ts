...
export function IdenticonStr({ input, size = 5, scale = 20 }: IdenticonProps): string {
  ...
  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      ...
      if (shape === "circle") {
        svgContent += `<circle cx="${x + scale / 2}" cy="${y + scale / 2}" r="${
          adjustedScale / 2
        ...
        svgContent += `<polygon points="${points}" fill="${color}" opacity="${opacity}" transform="rotate(${rotation}, ${
          x + scale / 2
        }, ${y + scale / 2})" />`;
      } else {
        // rect
        svgContent += `<rect x="${x + (scale - adjustedScale) / 2}" y="${
          y + (scale - adjustedScale) / 2
        }" width="${adjustedScale}" height="${adjustedScale}" fill="${color}" opacity="${opacity}" transform="rotate(${rotation}, ${
          x + scale / 2
        }, ${y + scale / 2})" />`;
      }
    }
  ...
  const svgString = `<svg width="${(size * scale) / 16}rem" height="${
    (size * scale) / 16
  }rem" viewBox="0 0 ${size * scale} ${size * scale}" xmlns="http://www.w3.org/2000/svg" style="background-color: rgb(${colors.r},${colors.g},${colors.b});" stroke="none">${svgContent}</svg>`;
