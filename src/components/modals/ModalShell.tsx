...
export function ModalShell({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn("overflow-y-auto top-[10vh] min-h-[50vh] max-w-2xl", contentClassName, className)}
        onPointerDownOutside={handlePointerDownOutside}
        onEscapeKeyDown={handleEscapeKeyDown}
      >
        {/* Navigation Button - positioned absolutely so it doesn't interfere with content layout */}
        {canGoBack && onBack && (
          <Button variant="outline" size="sm" className="top-4 left-4 z-10" title="back" onClick={onBack}>
