...
export class RemoteSearchFuzzyCache<TResult = any> {
  ...
  private async performSearch(): Promise<void> {
    console.log("performSearch called - agent:", !!this.agent, "searchTerm:", this._searchTerm);
    
    if (!this.agent) {
      console.log("No agent, returning empty results");
    ...
    try {
      ...
      console.log("Cache status - isStale:", isStale, "cache length:", this.cache.allItems.length);
      ...
      if (!isStale && Date.now() - this.cache.lastCheck > 2000) {
        ...
        console.log("Update check complete - isStale:", isStale);
      }

      // Fetch fresh data if needed
      if (isStale) {
        console.log("Data is stale, calling fetchAll...");
        this.cache.allItems = await this.agent.fetchAll({ signal: this.controller.signal });
        console.log("fetchAll complete, got", this.cache.allItems.length, "items");
      } else {
