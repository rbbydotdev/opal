...
import { EMPTY_SEARCH_RESULT, IRemoteAuthAgentSearch } from "@/data/useRemoteSearch";
import fuzzysort from "fuzzysort";
import { useEffect, useRef, useState } from "react";

export function useRepoSearchLegacy(agent: IRemoteAuthAgentSearch<Repo> | null, searchTerm: string) {
  ...
  useEffect(() => {
    ...
    const performSearch = async () => {
      ...
      try {
        ...
        if (isStale) {
          cache.current.allRepos = await agent.fetchAll({ signal: controller.signal });
        }
